...
type Tab = "overview" | "facturas-meli";
...
export default function FinanzasPage() {
  ...
  useEffect(() => {
    if (unit !== "unidrop" && tab === "facturas-meli") setTab("overview");
  }, [unit, tab]);
  ...
  const facturasTabEnabled = unit === "unidrop";
    ...
    <>
      <Topbar
        ...
        subtitle={
          unit === "unistore"
            ...
            : "ContabilliumInvoice (dev) · cobranzas Talo · facturas por suscripciones MELI"
        }
      />
      ...
      <div className="flex-1 px-8 py-6 overflow-y-auto">
        ...
        <div className="flex items-center gap-1 border-b border-border mb-6">
          ...
          <TabBtn
            active={tab === "facturas-meli"}
            disabled={!facturasTabEnabled}
            disabledHint={!facturasTabEnabled ? "Solo disponible en Unidrop" : undefined}
            onClick={() => facturasTabEnabled && setTab("facturas-meli")}
            label="Facturas suscripciones MELI"
          />
        </div>
        ...
        {tab === "facturas-meli" && facturasTabEnabled && <FacturasSuscripcionesMeliPanel />}
      </div>
    </>
  );
}
...
function TabBtn({
  active, onClick, label, disabled, disabledHint,
}: {
  active: boolean;
  onClick: () => void;
  label: string;
