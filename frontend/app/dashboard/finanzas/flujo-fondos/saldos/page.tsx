  ...
  return (
    <PageWrapper>
      ...
      <div className="rounded-xl border border-border bg-surface overflow-hidden">
        <div className="px-5 py-3 border-b border-border"><h2 className="text-sm font-bold">Historial de saldos cargados</h2></div>
        {history.isLoading ? <LoadingState /> : history.error ? <ErrorState message={(history.error as Error).message} /> : history.data && history.data.items.length === 0 ? <EmptyState /> : (
          <table className="w-full text-sm">
            <thead className="bg-soft border-b border-border text-[10px] uppercase tracking-wider text-text-muted">
              <tr><th className="text-left px-3 py-2">Fecha</th><th className="text-left px-3 py-2">Banco</th><th className="text-right px-3 py-2">Saldo</th><th className="text-left px-3 py-2">Fuente</th></tr>
            </thead>
            <tbody>
              {history.data?.items.map((s) => (
                <tr key={s.id} className="border-t border-border hover:bg-soft">
                  <td className="px-3 py-2 whitespace-nowrap">{fmtDate(s.fecha)}</td>
                  <td className="px-3 py-2 font-semibold">{s.banco_nombre ?? `#${s.banco_id}`}</td>
                  <td className="px-3 py-2 text-right font-semibold">{fmtArs(s.saldo)}</td>
                  <td className="px-3 py-2 text-text-muted text-xs">{s.fuente}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function SaldoModal({ bancos, onClose, onSaved }: { bancos: Banco[]; onClose: () => void; onSaved: () => void }) {
  const [form, setForm] = useState({
    ...
    banco_id: bancos[0]?.id ?? 0, saldo: "", fuente: "manual",
  });
  const m = useMutation({
