...
Todo desde Unistore (TN + digip). Unidrop podra adaptarse despues.
"""
...
def sku_optimizer_overview() -> dict:
    """Top recomendaciones accionables por categoria."""
    ...
    # 2) REPOSICION URGENTE - velocidad de venta + stock bajo
    try:
        repo_rows = q(eng, """
            WITH velocidad AS (
                SELECT oi.sku,
    ...
    try:
        pricing_rows = q(eng, """
            ...
            FROM ventas
            WHERE units_30d >= units_prev30d * 0.9   -- demanda estable o creciendo
              AND units_30d <= units_prev30d * 1.5   -- no es viral - estable
    ...
    pricing = [{
        ...
        "precio_sugerido": round(float(r[4] or 0) * 1.05, 0),  # +5% test
